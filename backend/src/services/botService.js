...
export const initTelegramBot = () => {
    ...
    bot.setMyCommands([
        ...
        { command: 'modules', description: 'View and use your voice modules' },
    ...
    bot.onText(/\/start (.+)/, async (msg, match) => {
        ...
        try {
            ...
            user.telegram.linkingCode = undefined;
            await user.save();
    ...
    bot.on('message', async (msg) => {
        ...
        if (text === 'MODULES') return handleModules(chatId);
        ...
        if (state?.step === 'CONFIRMING_NL_CALL') {
            if (text === 'YES') {
                ...
                delete userStates[chatId];
                await bot.sendMessage(chatId, '⚡ **INITIATING_CALL...**');
                return await triggerCall(chatId, callState);
            } else {
                delete userStates[chatId];
                return sendMainMenu(chatId, '✖️ **CALL DISCARDED.**');
            }
        }
        ...
        if (state) {
            ...
            if (state.step === 'AWAITING_NAME') {
                ...
                state.phone = phone;
                state.moduleId = state.moduleId; // preserve from before
                state.step = 'READY';
    ...
    bot.on('callback_query', async (query) => {
        ...
        try {
            if (data.startsWith('view_mod_')) {
                const moduleId = data.split('_')[2];
                const module = await Module.findById(moduleId);
                if (!module) return bot.answerCallbackQuery(query.id, { text: 'ERROR: Not found' });
                bot.editMessageText(`MODULE: ${module.name.toUpperCase()}\nTYPE: ${module.type.toUpperCase()}`, {
                    chat_id: chatId,
                    message_id: query.message.message_id,
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: '[ START CALL ]', callback_data: `start_call_init_${moduleId}` }],
                            [{ text: '[ BACK TO LIST ]', callback_data: 'list_modules' }]
                        ]
                    }
                });
            }
            ...
            if (data === 'list_modules') {
                ...
                const modules = await Module.find({ userId: user._id, isDeleted: false });
                const buttons = modules.map(m => ([{ text: `[${m.name.toUpperCase()}]`, callback_data: `view_mod_${m._id}` }]));
                bot.editMessageText('SELECT MODULE:', {
            ...
            if (data.startsWith('start_call_init_')) {
                userStates[chatId] = { step: 'AWAITING_NAME', moduleId: data.split('_')[3] };
            ...
            if (data.startsWith('set_lang_')) {
                ...
                bot.editMessageText('⚡ **SYSTEM: INITIATING_CALL...**', { chat_id: chatId, message_id: query.message.message_id });
    ...
    async function handleModules(chatId) {
        try {
            const user = await User.findOne({ 'telegram.chatId': chatId.toString() });
            if (!user) return bot.sendMessage(chatId, 'NOTICE: Authentication required. Link account first.');
            const modules = await Module.find({ userId: user._id, isDeleted: false });
            if (modules.length === 0) return bot.sendMessage(chatId, 'NOTICE: No voice modules found.');
            const buttons = modules.map(m => ([{ text: `[${m.name.toUpperCase()}]`, callback_data: `view_mod_${m._id}` }]));
            bot.sendMessage(chatId, 'SYSTEM: SELECT MODULE', { reply_markup: { inline_keyboard: buttons } });
        } catch (error) {
            logger.error('Bot Modules Error:', error);
            bot.sendMessage(chatId, 'ERROR: Failed to retrieve modules.');
        }
    }
    ...
    async function handleWorkspaces(chatId) {
        try {
            ...
            const buttons = workspaces.map(ws => ([{
                text: `${ws._id.toString() === user.currentWorkspace?._id.toString() ? '🟢 ' : ''}${ws.name.toUpperCase()}`,
                callback_data: `switch_ws_${ws._id}`
            }]));

            bot.sendMessage(chatId, ' **SELECT ACTIVE WORKSPACE**', { reply_markup: { inline_keyboard: buttons } });
        } catch (error) {
    ...
    function handleHelp(chatId) {
        const helpText = ` **Voicely INTELLIGENT BOT GUIDE**\n---\n` +
            `1. **Natural Language**: Just say "Call John at 9876543210 regarding the booking" to start a call instantly.\n` +
            `2. **Modules**: Use the button to browse specific voice modules manually.\n` +
            `3. **Workspaces**: Switch your context (Real Estate, Medical, etc.) using the WORKSPACES button.\n` +
            `4. **Updates**: Get automated summaries as soon as a call ends.\n\n` +
...
async function triggerCall(chatId, state) {
    try {
        ...
        if (!state.moduleId) throw new Error('No voice module selected.');

        logger.info(`Bot Triggering Call: [Customer: ${customerName}] [Phone: ${phoneNumber}] [Module: ${state.moduleId}]`);
        ...
        bot.sendMessage(chatId, `🚀 **CALL_STATUS: INITIATED**\nSID: ${twilioCall.sid}\nDEST: ${phoneNumber}`, {
            reply_markup: mainMenuKeyboard
        });
    } catch (e) {
