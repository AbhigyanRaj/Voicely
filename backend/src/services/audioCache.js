...
export async function getOrGenerateAudio(text, voiceType = 'RACHEL', options = {}) {
  ...
  if (fs.existsSync(localPath)) {
    ...
    AudioFile.findByTextHash(textHash, voiceType)
      ...
      .catch(err => logger.error('Error updating audio usage', err));
  ...
  logger.info(`Generating audio with ElevenLabs for: ${text.substring(0, 30)}...`);
...
export async function preGenerateModuleAudio(module, voiceType = 'RACHEL') {
  logger.info(`Pre-generating audio for module: ${module.name}`);
  ...
  try {
    ...
    logger.success('Module audio pre-generation complete');
    return results;
  } catch (error) {
    logger.error('Module audio pre-generation failed', error);
    throw error;
  }
}
...
export async function initializeSharedAudioLibrary(voiceType = 'RACHEL') {
  ...
  for (const phrase of sharedPhrases) {
    try {
      ...
      logger.error(`Failed to generate shared audio: ${phrase.text}`, error);
    }
  }

  logger.success(`Shared audio library initialized: ${generated} generated, ${cached} cached`);
...
export async function cleanupOldAudio() {
  ...
  for (const audioFile of oldFiles) {
    try {
      ...
      logger.error(`Failed to cleanup audio file: ${audioFile.localPath}`, error);
    }
  }

  logger.success(`Cleanup complete: ${deleted} files removed from local storage`);
